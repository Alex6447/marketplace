...
- [ ] Модель данных + миграции. _<!-- частично: пустая БД `marketplace` создана на localhost:5432, DATABASE_URL в .env; схемы/таблиц и миграций ещё нет -->_
- [ ] CRUD: проект → товар → загрузка фото/описания.
- [ ] Стадия [2] генерация идей (LLM).
- [ ] Стадия [3] визуальные концепции (LLM, JSON-контракт).
