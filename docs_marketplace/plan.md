...
> Базируется на [docs_marketplace/idea.md](idea.md). Документ описывает архитектуру, технологический стек, пайплайн генерации, модель данных и поэтапный план работ от MVP до production.
...
- [x] Сравнение подходов на реальных товарах заказчика, выбор дефолта. _<!-- ВЫПОЛНЕНО локально (2026-06-24): прогон на 4 реальных фото товаров (кроссовки/наушники/2 бутылки, скачаны из Wikimedia Commons → products/) через ЛОКАЛЬНЫЙ ComfyUI+Flux.1 Kontext (image_provider='comfyui', matting='simple'), т.к. у Gemini free-tier=0 на image-gen. Результат (concept.json, size 1024², seed 7): composite лидирует 4/4 по сохранности товара — product_fidelity edit 0.61–0.99 (заметный дрейф Kontext Q4) vs composite 1.00 (пиксели товара 1:1 по построению), фон composite меняется сильнее, локально ещё и ~2× быстрее (~95с vs ~180с). РЕКОМЕНДАЦИЯ/ВЫБОР ДЕФОЛТА: режим [5] 'composite' (gold standard) при условии качественного выреза [4]; edit (Kontext) — альтернатива. ВАЖНЫЙ нюанс: качество composite упирается в matting — SimpleMattingProvider (кеинг по белому) хорошо вырезает НЕпрозрачные товары (кроссовки/наушники/aperol), но даёт артефакты на прозрачном/белом товаре (бутылка evian просвечивает) → для таких товаров нужен BiRefNet (Этап 6, стадия [4]). Отчёт compare_out/report.html. --- НИЖЕ описание инструмента: ИНСТРУМЕНТ ГОТОВ. Харнесс сравнения режимов стадии [5] edit↔composite: чистый модуль marketplace_shared.pipeline.compare (метрики сохранности товара/изменения фона на Pillow+ImageStat по маске [4]: product_fidelity=1−mean|diff| в области товара, background_change вне её; прогон обоих режимов run_edit_approach/run_composite_approach через ImageProvider+MattingProvider из реестра; compare_product строит маску [4] один раз и шарит её обоим режимам — честная разметка; recommend_default — эвристика «сначала сохранность товара, затем изменение фона», помечена как подсказка; render_report_html — самодостаточный HTML с картинками data-URI и таблицей метрик/времени/расхода/промтов). CLI scripts/compare_approaches.py (--photo/--photos-dir/--concept[CardConcept|CardSetConcepts]/--out/--size/--seed/--brand-style; провайдеры по env, дефолт echo/simple — гоняется офлайн; stdout/stderr→utf-8 для Windows). ruff чисто. Проверено офлайн (echo/simple) на 2 синтетических товарах: edit (echo=no-op) fidelity 1.0/фон 0.0, composite fidelity 1.0/фон 0.08–0.12 — рекомендация «composite»; HTML с 6 встроенными картинками и result-PNG записаны. Реальное сравнение качества: `IMAGE_PROVIDER=gemini GEMINI_API_KEY=... uv run python scripts/compare_approaches.py --photos-dir ./products --concept concept.json --size 1024x1024`; после прогона на товарах заказчика человек выбирает дефолт и проставляет [x]. -->_
...
- [x] Рендер текстовых блоков из JSON концепции поверх изображения. _<!-- Маппинг pipeline/textoverlay.py: concept_to_render_blocks (чистая) переводит CardConcept.text_blocks → RenderBlock по шаблону МП — роль задаёт кегль (доля высоты холста: headline 0.085/bold, subheadline/bullet/caption/spec/badge), выравнивание и позицию-фолбэк; позиция берётся из концепции, если попадает в сетку 3×3, иначе дефолт роли; bullet'ы сливаются в один маркированный блок (защита от наложения при якорном размещении); итог усекается до template.max_text_blocks; badge — белый текст на плашке #c0392b с паддингом. build_card_render_request (холст+safe_zone из шаблона) и async render_card_text (через TextRenderer). Воркер stages.run_card_text_overlay: берёт image_s3_key версии [5] + концепцию карточки, рендерит выбранным TEXT_RENDERER по шаблону, контент-адресуемо (digest = изображение + блоки + шаблон + рендерер → cache/text_overlay/{d}.png, miss→hit), пишет CardVersion.final_s3_key и gen_params_json.text_overlay (не затирая параметры [5]). Celery: JOB_CARD_TEXT/TASK_CARD_TEXT (shared/jobs.py), card_text_overlay_task (владеет Job, lifecycle running→success, stage='text_overlay'), enqueue_card_text (name-based). API: POST /card-versions/{id}/text (202+Job; 404 нет версии; 409 нет изображения [5]; template_key опц.); CardVersionRead получил final_url (presigned). ruff чисто. Проверено офлайн: маппинг (headline>subheadline, badge-плашка, 3 bullet'а→1 блок, масштаб кегля 85→122 на большем холсте, лимит шаблона, фолбэк позиции, пустой блок отброшен); eager E2E на marketplace_test+фейк-хранилище ([5]→версия→[6] miss→hit, final_s3_key=cache/text_overlay/, текст нанесён, размер холста, смена шаблона→новый адрес, gen_params [5] сохранены, нет изображения→ValueError); Celery-задача eager (Job success/progress 100/stage); API ASGI (404/409/202 + enqueue с template). NB: реальный Playwright-рендер требует extra render + Chromium; дефолт pillow. Интеграция [6] в общий канвас набора (после [5]) — при необходимости позже; сейчас стадия вызывается отдельным эндпоинтом на версию. -->_

**Этап 3 завершён** — детерминированное наложение текста (движок Playwright/Pillow, шаблоны Ozon/WB/ЯМ с safe-zones, рендер блоков концепции [3] поверх изображения [5] как стадия [6] с очередью/кэшем/API).

### Этап 4 — Цикл обратной связи (1–2 недели)
- [x] LLM-парсер свободного фидбэка → действие + стадия + дельта-параметры. _<!-- Провайдеро-независимая стадия [9] — пакет marketplace_shared.pipeline (feedback.py): Pydantic-контракты результата ParsedFeedback (summary + target_stage + action + changes[] + confidence + notes) — он же единственный источник JSON Schema (FEEDBACK_RESPONSE_SCHEMA) для structured-вывода LLM и валидации; перечисления StrEnum FeedbackStage (concept[3]/image[5]/text[6]/ideas[2]/unknown — какую стадию перегенерировать), FeedbackActionType (adjust/regenerate), ChangeOperation (set/add/remove/modify); FeedbackChange (field на языке полей концепции background/composition/color_palette/text_blocks/… или параметров seed/model + operation + instruction + value) — это и есть дельта-параметры. Вход FeedbackInput (свободный текст + текущая CardConcept[3] + brand_style); build_feedback_request (чистая, RU system-промт: разбор по МИНИМАЛЬНОЙ стадии + инвариант «товар не искажать» → запрет в notes) и parse_feedback(provider, input) (вызов LLMProvider.complete, валидация, ProviderError если нет structured-data). API-роутер apps/api/.../routers/feedback.py: POST /card-versions/{id}/feedback (синхронно, лёгкая LLM-стадия как [2]/[3]; собирает контекст карточки [3]+стиль бренда, разбирает фидбэк, пишет Feedback.text + parsed_action_json; 404 нет версии; 422 пустой текст; 502 ProviderError) и GET /card-versions/{id}/feedback (история). DTO FeedbackCreate/FeedbackRead (parsed_action ← parsed_action_json). echo даёт детерминированный валидный разбор (enum[0]=concept). ruff чисто. Проверено: офлайн-smoke пайплайна на echo (схема/$defs/enum, build_request с контекстом концепции, без концепции контекст опускается, parse → ParsedFeedback) + end-to-end ASGI на marketplace_test (404/422/201 с сохранённым parsed_action/список из 2/404-список). Перегенерация адресуемой стадии по разобранному действию (постановка в очередь) — следующий пункт Этапа 4. -->_
- [x] Перегенерация нужной стадии, история версий. _<!-- По разобранному фидбэку [9] перегенерируется ТОЛЬКО адресуемая стадия, всегда создаётся новая CardVersion (история не перетирается). Чистый аппликатор дельт marketplace_shared.pipeline.regen: apply_changes_to_concept(concept, changes)→(новая концепция, непринятые) детерминированно применяет value-несущие правки к полям концепции [3] (скаляры background/composition/product_placement set|modify; списки color_palette/icons/infographics/must_have/must_not_have add|remove|set; text_blocks add|remove), исходник не мутирует, правки-инструкции без value возвращаются как непринятые (под LLM-путь); extract_image_overrides→{seed,model} для стадии [5]. Оркестрация в worker stages.run_feedback_regeneration: target_stage=text [6] → новая версия НАСЛЕДУЕТ image_s3_key исходной (товар 1:1, изображение не пересобирается) + свежее наложение текста по обновлённой концепции; target_stage=image [5]/concept [3] → пересборка изображения через run_card_image (контент-адресуемый кэш сам решает, звать ли провайдера; seed/model из дельт как override; для concept дельты применяются к Card.concept_json → digest меняется → пересчёт); ideas [2]/unknown → ValueError (ручное решение). Версия получает gen_params.feedback (feedback_id/стадия/действие/исходная версия/непринятые правки). Celery: JOB_FEEDBACK_REGEN/TASK_FEEDBACK_REGEN (shared/jobs.py), feedback_regen_task (владеет Job, lifecycle), enqueue_feedback_regen (name-based). API: POST /feedback/{id}/regenerate (202+Job; 404 нет фидбэка; 409 не разобран или стадия ideas/unknown; template_key опц.→из исходной версии). DTO FeedbackRegenerateRequest. ruff чисто. Проверено офлайн (echo/simple/pillow): smoke аппликатора (set/add/remove/text_blocks/непринятые/overrides, исходник не мутирует) + E2E на marketplace_test с фейк-хранилищем (v1 image → text→v2 с наследованием image 1:1 и новым final + блок добавлен в концепцию; image seed=7→v3 пересобрано; concept фон→v4 концепция изменена; unknown→ValueError; история версий [1,2,3,4]); ASGI-smoke (404/409-не разобран/409-unknown/202 с enqueue+template_key, маршрут в OpenAPI); задача зарегистрирована в воркере. UI просмотра версий/ввода фидбэка/сравнения — следующий пункт Этапа 4. -->_
- [x] UI: просмотр версий, ввод фидбэка, сравнение вариантов. _<!-- Панель «Студия карточек» (стадии [5]/[6]/[9]) apps/frontend/src/components/product/VersionsPanel.tsx, подключена в ProductView после ConceptsPanel (гейтится на наличие концепций [3] — в ProductView добавлен общий запрос ["cards", productId] для hasConcepts, дедуплицируется с панелями). На карточку — CardStudio: переключатель режима [5] edit/composite + «Сгенерировать изображение / Новая версия» (POST /cards/{id}/generate → Job → опрос до терминала, прогресс-бар), галерея версий (GET /cards/{id}/versions) сеткой sm:2/xl:3 — это и есть сравнение вариантов. VersionCard: бейджи v{no}/mode/«с текстом»/кэш, превью object-contain с fallback, переключатель вида [5] фон ↔ [6] текст (если есть final_url), сворачиваемый блок «Текст и правки». Стадия [6]: выбор шаблона МП (константа MARKETPLACE_TEMPLATES = ozon-main/ozon-promo/wildberries-main/yandex_market-main) + «Нанести/Перерендерить текст» (POST /card-versions/{id}/text → Job). Стадия [9]: textarea фидбэка → «Разобрать фидбэк» (POST /card-versions/{id}/feedback), история (GET) с предпросмотром разбора ParsedFeedback (целевая стадия/действие/уверенность/summary/дельты changes/notes) и кнопкой «Перегенерировать стадию» (POST /feedback/{id}/regenerate → Job → новая версия) для авто-стадий concept/image/text; ideas/unknown — пометка «ручное решение». В lib/api.ts добавлены типы Job/CardVersion/Feedback/ParsedFeedback/FeedbackChange + функции generateCardImage/listCardVersions/renderCardText/submitFeedback/listFeedback/regenerateFromFeedback/getJob и хелпер waitForJob (опрос GET /jobs/{id} до success/failure с колбэком прогресса; SSE не используется — для UI достаточно опроса). Тема «ателье»/обе темы соблюдены (переиспользованы Panel/StageHeader/Badge/RoleTag/Field/Textarea/states). Проверки: prettier чисто, eslint 0 ошибок (3 предсуществующих warning не в новых файлах), tsc -b чисто, vite build успешен (1648 модулей). NB: живой E2E прогон стадий [5]/[6]/[9] требует поднятого стека (postgres/redis/minio + worker, для реальных картинок GEMINI_API_KEY + IMAGE_PROVIDER=gemini); без worker'а задачи остаются pending и UI показывает прогресс в ожидании — это инфраструктурный аспект, не логика UI. -->_

**Этап 4 завершён** — цикл обратной связи: LLM-парсер свободного фидбэка [9] (действие/стадия/дельты), перегенерация только адресуемой стадии [3]/[5]/[6] с историей версий, и UI «Студия карточек» (генерация изображения [5], наложение текста [6], ввод фидбэка и перегенерация по разобранному действию [9], сравнение версий).

### Этап 5 — QA, экспорт, полировка (1–2 недели)
- [x] Авто-QA (товар на месте, читаемость, размеры). _<!-- Стадия [7]. Провайдеро-независимая чистая логика marketplace_shared.pipeline.qa (только Pillow, без сети/torch): контракты QaCheck/QaReport (status pass/warn/fail/info/skipped, агрегат) + отдельные проверки check_dimensions (соотношение сторон под МП с допуском + минимальный размер шаблона), check_text_size (минимальный кегль блоков [6] относительно высоты холста), check_text_contrast (WCAG-контраст текста к фону — грубо по среднему цвету изображения; для бейджа — к цвету плашки), check_product_fidelity (сохранность товара: режим edit — попиксельно через compute_metrics из compare.py по маске [4]; composite — info «1:1 по построению»; нет маски/фото — skipped), check_white_background (белизна фона по рамке для шаблонов white_background_required), check_forbidden_elements (must_not_have концепции — info, vision-проверка отложена на Этап 6); build_qa_report собирает отчёт по концепции [3] + шаблону + байтам final[6]/image[5]/source/mask. Пороги вынесены константами. Хранение — CardVersion.qa_report_json (поле уже было в модели — миграция не нужна). API-роутер apps/api/.../routers/qa.py (синхронно, как лёгкие стадии — QA локальная, без провайдеров): POST /card-versions/{id}/qa (грузит байты из MinIO в threadpool → build_qa_report → пишет qa_report_json; 404 нет версии; 409 нет изображения; 400 неизвестный template_key; шаблон из запроса→из стадии [6] версии→дефолт) и GET /card-versions/{id}/qa (последний отчёт; 404 если не запускалось). DTO QaRunRequest; CardVersionRead расширен qa_report. Frontend: в студии карточек (VersionsPanel) блок «Авто-QA [7]» в развёрнутой версии — кнопка «Проверить/Перепроверить качество» (POST qa, синхронно), вывод отчёта (агрегат-пилюля + список проверок со статус-иконками/деталями/score), QA-бейдж в шапке версии; типы QaReport/QaCheck/QaStatus + runQa/getQa в lib/api.ts; QaPill с классами по статусу (success/primary/destructive/muted — у общего Badge нет warning-тона). ruff чисто; офлайн-smoke build_qa_report (идеальная Ozon-карточка→pass; тёмная 1000×500→fail на соотношении/контрасте/белом фоне; fidelity edit=1.0 pass / composite info; JSON-сериализация); API импортируется, маршруты POST/GET в OpenAPI; eslint 0 ошибок, tsc -b чисто, vite build (1648 модулей). NB: эмбеддинги/SSIM по региону товара и vision-QA запрещённых элементов — улучшение Этапа 6 (локальные модели). -->_
- [x] Скачивание/экспорт комплекта (zip, под форматы МП). _<!-- Завершает пользовательский путь (9-й пункт idea.md «скачивание»). Роутер apps/api/.../routers/export.py: GET /card-versions/{id}/download (отдаёт PNG готового варианта — финал с текстом [6], иначе изображение [5]; 404 нет версии; 409 нет изображения; Content-Disposition attachment) и GET /card-sets/{id}/export (zip всего комплекта: по последней готовой версии каждой карточки набора — max version_no с изображением, предпочтительно с текстом — + manifest.json с метаданными {filename/role/order/version_no/has_text/template/qa_status}; 404 набор; 409 если нет ни одной готовой карточки). Чистая упаковка build_export_archive(entries, manifest)→bytes (zipfile+json, уникализация совпадающих имён, детерминированно — без таймстампов; имя файла {order:02d}-{slug(role)}.png, slug латиница/цифры/дефис, пустая роль→card). Сбор байтов из MinIO — в threadpool (boto3 синхронный). Frontend (VersionsPanel): кнопка «Скачать комплект» (ссылка-zip на /card-sets/{id}/export) в шапке студии при наличии карточек; иконка-ссылка «Скачать PNG» (/card-versions/{id}/download) в шапке каждой версии; URL-хелперы cardSetExportUrl/cardVersionDownloadUrl в lib/api.ts. ruff чисто; офлайн-smoke упаковки (уникализация имён, manifest, slug) + маршруты в OpenAPI; eslint 0 ошибок, tsc, vite build (1648 модулей); живая ASGI-проверка in-process на данных прошлого E2E (поднятый MinIO+нативный PG): export→200 application/zip с 01-card.png+manifest.json, download→200 image/png валидный PNG. NB: «под форматы МП» — имена/манифест несут роль и шаблон версии; ре-рендер под другой МП on-the-fly не делается (берётся уже готовый артефакт нужного шаблона). -->_
- [x] Обработка ошибок, ретраи, логирование, наблюдаемость (Langfuse/OTel + учёт стоимости). _<!-- Сделано в 2 коммита. ЧАСТЬ A (ретраи/ошибки/логи): единый хелпер worker run_job_task (tasks.py) оборачивает стадии [4]/[5]/[6]/[9] — статус Job, логи стадии (старт/финиш/длительность), трейс-span; транзиентные ошибки провайдера (новый TransientProviderError + стандартные сетевые/httpx + эвристика по тексту: timeout/429/5xx/connection reset) авто-повторяются с экспонентой backoff (2/4/8…≤30с, до 3 попыток) и статусом Job 'retry' (mark_retrying, не терминальный), постоянные → mark_failure; все 4 задачи переведены на хелпер. Централизованное логирование marketplace_shared/log.py (configure_logging, LOG_LEVEL из env, идемпотентно) в API (main.py) и worker (celery_app.py, worker_hijack_root_logger=False). ЧАСТЬ B (наблюдаемость+стоимость): пакет marketplace_shared.observability — cost.py (ориентировочные ценовые таблицы LLM per-Mtok / image per-image 2026; estimate_llm_cost/estimate_image_cost; apply_estimated_cost обогащает Usage.cost_usd оценкой с флагом extra.cost_estimated, если провайдер не дал), tracing.py (провайдеро-независимый Tracer: NoOpTracer дефолт без накладных / LoggingTracer / каркас langfuse|otel по OBSERVABILITY_PROVIDER с безопасной деградацией до logging — внешние клиенты тяжёлые, Этап 6). Стадия [5] пишет usage с оценкой стоимости в gen_params. Эндпоинт GET /card-sets/{id}/cost (routers/cost.py, чистая summarize_costs) агрегирует стоимость генераций [5] по версиям набора (риск «дорогая генерация», раздел 8) → CostSummary (total/image_generations/estimated/by_card). Frontend: бейдж стоимости комплекта (CostBadge, getCardSetCost) рядом с «Скачать комплект» в студии, инвалидация при генерации/перегенерации. .env_example: LOG_LEVEL/OBSERVABILITY_PROVIDER. Проверки: ruff чисто; офлайн-smoke (A: _is_transient транзиентные vs постоянные, идемпотентность логов, JOB_RETRY; B: cost LLM/image/apply, tracer none→NoOp/langfuse→деградация, агрегация); eslint 0 ошибок/tsc/vite build; живая проверка (поднятый стек, eager): success-путь run_job_task (лог+Job success+v3 с usage.cost_usd=0.04 estimated), failure-путь (лог+mark_failure), cost-эндпоинт 200 с агрегатом. NB: реальные клиенты Langfuse/OTel и учёт стоимости LLM-стадий [2]/[3] (usage не привязан к версии) — расширение позже. -->_

**Этап 5 завершён** — авто-QA [7] (товар/читаемость/размеры/белый фон), экспорт комплекта в zip + скачивание версии, и обработка ошибок/ретраи транзиентных сбоев/структурное логирование/наблюдаемость (учёт стоимости + провайдеро-независимый трейсинг).
...
- [x] Локальная реализация `ImageProvider`: ComfyUI (Flux/Qwen-Image-Edit/SDXL), BiRefNet/SAM2. _<!-- СДЕЛАНО (2026-06-24) на RTX 3060 12ГБ, всё в marketplace_shared.providers.comfyui (HTTP-клиент к локальному ComfyUI, без torch в shared). [5] ComfyUIImageProvider: edit = граф Flux.1 Kontext (GGUF→VAEEncode реф.→ReferenceLatent→FluxGuidance→KSampler→VAEDecode), generate = Flux txt2img (EmptySD3LatentImage); IMAGE_PROVIDER='comfyui'. [4] BiRefNetMattingProvider: граф AutoDownloadBiRefNetModel→GetMaskByBiRefNet→MaskToImage (нода ComfyUI_BiRefNet_ll, веса авто-скачиваются), вырез RGBA собирается из маски на Pillow; MATTING_PROVIDER='birefnet'. Конфиг COMFYUI_*/COMFYUI_BIREFNET_MODEL в config.py/.env_example, registry обновлён. pytest packages/shared/tests/test_comfyui_provider.py (12 шт.: графы edit/generate/mask, регистрация image+matting, parse_size, сборка выреза). Проверено вживую: edit ~180с (товар сохранён, фон сменён), BiRefNet ~28с даёт ЧИСТУЮ сплошную маску прозрачной бутылки evian (simple-matting тут проваливался) — решает нюанс composite из задачи Этапа 2. Стенд (вне репо) D:\AI\ComfyUI — см. память comfyui-local-stack. ОПЦИОНАЛЬНО позже: Qwen-Image-Edit/SDXL как доп. модели, SAM2 как альт. matting (в registry 'sam2' пока ProviderNotImplemented). -->_
- [x] Локальная реализация `LLMProvider`: vLLM/Ollama (напр. Qwen2.5-VL) для идей/концепций/фидбэка. _<!-- СДЕЛАНО (2026-06-24) на Ollama: marketplace_shared.providers.ollama.OllamaLLMProvider (HTTP /api/chat, без ML в shared). Структурный вывод стадий [2]/[3]/[9] — через нативный format=<JSON Schema> (grammar-constrained), парсинг устойчив к ```-ограждениям/преамбуле (_extract_json). LLM_PROVIDER='ollama', конфиг OLLAMA_URL/OLLAMA_MODEL (config.py/.env_example), registry обновлён. pytest packages/shared/tests/test_ollama_provider.py (8 шт.: payload/format, извлечение JSON, регистрация, приоритет llm_model). Проверено вживую: стадия [2] generate_ideas через gemma4:latest (8B Q4) → 6 валидных слайдов ProductIdeas, грамотный RU, ~32с, токены учтены. NB (Windows): порт Ollama 11434 попадает в зарезервированный диапазон Hyper-V/WSL (11370–11469) → поднимать на 11500 (OLLAMA_HOST=127.0.0.1:11500) и указывать в OLLAMA_URL. Думающие модели: think НЕ выставляем (ломает grammar схемы), закладываем запас num_predict. vLLM — альтернативный backend при необходимости. -->_
- [x] Монтирование и версионирование весов моделей (том + кэш). _<!-- СДЕЛАНО (2026-06-24): docs_marketplace/local-models.md — все локальные веса запинены (источник HF + точное имя файла + размер + SHA256): Flux.1 Kontext Q4_K_M, T5 Q5_K_M, clip_l, Flux VAE (ae), BiRefNet General + модели Ollama (gemma4/gpt-oss по дайджесту manifest). Стратегия том+кэш: веса не в образе, монтируются именованным томом (comfyui-models→/app/ComfyUI/models, ollama-models→/root/.ollama), переживают пересборку; команда проверки целостности sha256sum против пинов. Реальный docker-compose с этими томами/профилями — Этап 7. -->_
- [x] Конфиг режима (hosted / local / hybrid) на уровне окружения/проекта. _<!-- СДЕЛАНО: архитектурно три режима достигаются переключением провайдеров через env (LLM_PROVIDER/IMAGE_PROVIDER/MATTING_PROVIDER), пайплайн их не знает (registry). Все backends реализованы: hosted (anthropic/gemini), local (ollama/comfyui/birefnet), echo (офлайн). В .env_example добавлены готовые ПРЕСЕТЫ режимов hosted/local/hybrid/offline (точные строки для копирования) + DEPLOYMENT_MODE как справочная метка. Конфиг — на уровне окружения/проекта (разные .env → разные режимы на одном сервере). -->_
- [ ] Сравнение качества/скорости local vs hosted на товарах заказчика. _<!-- ЧАСТИЧНО/ЗАБЛОКИРОВАНО: local-сторона измерена на RTX 3060 12ГБ (Этап 2-сравнение через ComfyUI: [5] edit ~180с/composite ~95с, [4] BiRefNet ~28с, LLM [2] gemma4 ~32с; качество — composite сохраняет товар 1:1, BiRefNet чист на прозрачных). Hosted-сторона недоступна: у текущего GEMINI_API_KEY free-tier=0 на image-gen (нужен биллинг). Для полного local-vs-hosted сравнения: включить биллинг Gemini (или дать ключ с биллингом) и прогнать scripts/compare_approaches.py обоими провайдерами + замерить LLM на anthropic. Инструмент готов, нужен платный hosted-доступ + товары заказчика. -->_

### Этап 7 — Размещение на сервере заказчика
> Выбор площадки/зоны (РФ/Европа/Китай) и помесячная стоимость при «модели по API» — [docs_marketplace/hosting.md](hosting.md).
