...
- [ ] Локальная реализация `ImageProvider`: ComfyUI (Flux/Qwen-Image-Edit/SDXL), BiRefNet/SAM2. _<!-- ЧАСТИЧНО (2026-06-24): ImageProvider на ComfyUI СДЕЛАН — marketplace_shared.providers.comfyui.ComfyUIImageProvider (HTTP-клиент к локальному ComfyUI, без torch в shared): edit = граф Flux.1 Kontext (GGUF-загрузчики→VAEEncode реф.→ReferenceLatent→FluxGuidance→KSampler→VAEDecode), generate = Flux txt2img (EmptySD3LatentImage). Зарегистрирован в registry (IMAGE_PROVIDER='comfyui'), конфиг COMFYUI_* в config.py/.env_example, pytest-тесты packages/shared/tests/test_comfyui_provider.py (8 шт., чистые графы+регистрация+parse_size). Проверено вживую на RTX 3060 12ГБ: edit ~180с, фон через 4/4 в сравнении выше. Стенд (вне репо): D:\AI\ComfyUI + Flux.1 Kontext Q4_K_M/T5 Q5/clip_l/VAE — см. память comfyui-local-stack. ОСТАЛОСЬ: matting через BiRefNet как проектный MattingProvider (нода ComfyUI_BiRefNet_ll УЖЕ установлена в стенде, но в registry 'birefnet' пока ProviderNotImplemented — сейчас используется 'simple'); Qwen-Image-Edit/SDXL опционально. -->_
