...
- [ ] Каркас FastAPI + React (TanStack Query, shadcn/ui), базовый CI, линтеры. _<!-- сделано и проверено: FastAPI-каркас (config.py на pydantic-settings, CORS, роутеры под /api + /healthz; ruff check/format чисто, OpenAPI отдаёт /api/health и /healthz). Написан каркас фронта apps/frontend (Vite+React18+TS, TanStack Query, shadcn/ui: Tailwind/CSS-переменные/Button, демо-запрос /api/health) + линтеры (ESLint flat + Prettier) + CI .github/workflows/ci.yml (python: uv+ruff; frontend: pnpm+eslint+prettier+tsc/build). НЕ проверено: pnpm install/build — Node стабильно получает ECONNRESET от registry.npmjs.org в этом окружении (curl даёт 200), сетевое ограничение. Нужен один `pnpm install` при доступе к npm → перегенерировать корневой pnpm-lock.yaml (сейчас пустой) и закоммитить, тогда чекбокс закрыть -->_
...
- [ ] Подключение editing-API (Gemini/Flux) — проверка «сохрани товар, смени фон».
...
- [ ] UI: создание проекта, ввод данных, просмотр идей и концепций.
